...
export async function GET(
  ...
  try {
    ...
    // ✅ FIX 4 — Use handleApiError instead of manual console.error + json
    // BUG: original code here used:
    //   console.error('Error fetching car:', error)
    //   return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
    //
    // That means if Prisma throws a P2025 (record not found via findUnique
    // with a malformed ID), you'd return 500 instead of the correct 404.
    // handleApiError catches all Prisma error codes properly.
    return handleApiError(error);
  }
}
